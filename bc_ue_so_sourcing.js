...
define([
    ...
    function buildViewModePickerHtml(lines) {
        ...
        return [
            ...
            '  if(!row) return null;',
            '  var el=row.querySelector("[id^=inpt_custcol_bc_linked_to],[id^=custcol_bc_linked_to],[name^=custcol_bc_linked_to]");',
            '  if(!el) return null;',
            '  var td=el;',
            '  while(td && td!==row){ if(td.tagName==="TD"||td.tagName==="TH") return td; td=td.parentNode; }',
            '  return null;',
            '}',
            ...
            '  if(idx>=0 && (idx+1) < row.children.length){',
            '    row.insertBefore(cell, row.children[idx+1]);',
            '  } else {',
